...
You can run a demo with `script demo.txt`. The docker-compose file (`compose.yaml`) indeed contains a volume declaration to mount the `demo.txt` file which can be directly used from the cli as below to iterate through a complete scenario and check that verify is running fine :
...
Here are some useful commands with docker compose:
    ...
    docker ps # to list the running containers with their health status
