...
The setup is done through a bom (bill of materials in maven pom). To use one of the last version of Cucumber compatible with our stack, we need to add some dependency management for JUnit and Mockito.
The versions of these two frameworks specified by SpringBoot are not recent enough for Cucumber.

We thus have to add the following dependencies in the POM file of the backend:
...
```xml
  <properties>
    ...
    <cucumber.version>7.31.0</cucumber.version> <!-- Last cucumber version compatible with SpringBoot 3.5.9 -->
  </properties>
...
...
    ...
    @Autowired // Spring/Cucumber setup: autowired the mock declared in the Config class
    private Bank bankMock;
...
## Testing a RestClient

It is also possible to test the RestClient that we use for calling REST routes (from the cli to the backend, and from the backend to the bank). 

To do so, we have to use a `RestClientTest` annotation on the test, passing the class of the RestClient under test as a parameter. 
This annotation will set up a Spring test container with only the RestClient and the necessary configuration to test it.
We also use `MockRestServiceServer` that will mock the server called by the RestClient, and the `BankProxy` implementation that we autowire.
...
```
Then, in each test, one define what will be returned by the mock server during the test. 
In the case of a successful payment, we return the right body (with the help of an `objectMapper`), the right status code and the right header. 
After the call to the `pay` method on the bankProxy under test, we can use assert methods as usual.
We can also check that all expectations defined on the mock server have been met with the `verify()` method.
...
The class contains all the tests to cover some exceptional cases, for example if the bank would return a 404 or 500 error code:
...
## Testing a RestClient that is not defined in the client component

In some cases, a RestClient is not defined in the client component, but in another one, as in the *cli*.
In that case, we need to give a specific `RestClient` for the testing part:
...
Then, in the test class, we have to import this configuration class with the `@Import` annotation:
